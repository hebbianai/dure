...
Read this before choosing a font for a piece of text. It defines the typeface
roles used with [SOUL](SOUL.md). Implementations are the authority for what
ships; this page explains the choice between mono and sans.
...
`@fontsource-variable/geist`, and [src/index.css](../src/index.css) is where the
two families are bound.
...
Mono does not mean 11px. The size belongs to the slot the text sits in. Keep the
size and line height defined by the existing component and its tokens; this
rule selects the typeface and weight rather than introducing a new size scale.
