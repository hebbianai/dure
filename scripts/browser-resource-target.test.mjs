...
test("shared commands use the explicit target among multiple resources", async () => {
  ...
  const report = await client.run(["get", "url", "--current"]);
...
test.each([
...
])("shared target reaches the existing %j contract", async (prefix, values, options) => {
  ...
  const actual = await scoped.run([...prefix, "--current", ...values, ...options]);
  assert.equal(actual.ok, true, JSON.stringify(actual));
  assert.deepEqual(scoped.calls[0], { kind: "list" });
...
])("shared inventory %j never invents a target", async (resources, code) => {
  const client = fixture({ catalog: { workspace_id: resource.workspace_id, resources } });
  const report = await client.run(["fill", "input", "must not run", "--current", ...flags]);
...
test("a resource replaced or moved after list cannot redirect any observation family", async () => {
  for (const observedResource of [{ ...resource, generation: "replacement" }, { ...resource, workspace_id: "foreign" }]) {
    for (const args of [["fill", "input", "must not run", ...flags], ["console"], ["dialog", "status", "--page", page.page_id], ["control", "--controller", "agent"]]) {
      ...
      const report = await client.run([...args, "--current"]);
...
test("shared selection cannot borrow a controller or retry uncertain input", async () => {
  const stale = fixture({ controller: { ...lease, epoch: "9" } });
  const refusal = await stale.run(["fill", "input", "text", "--current", ...flags]);
  ...
  for (const failAt of ["list", "observe", "action"]) {
    ...
    const report = await client.run(["fill", "input", "text", "--current", ...flags]);
...
test("workspace-targeted references retain the original document and resource checks", async () => {
  ...
  const report = await client.run(["fill", reference, "한글", "--current", ...flags]);
  assert.equal(report.ok, true, JSON.stringify(report));
  ...
  const refused = await conflicting.run(["fill", foreign, "text", "--current", ...flags]);
...
test("contradictory target grammars and catalog-only commands fail before backend selection", async () => {
  for (const args of [
    ["show", "clear", "--current"],
    ["show", "--resource", "clear", "--current"],
    ["snapshot", "--current", "--current"],
    ["snapshot", "--worktree", "branch:main"],
    ["tab", "profile", "list", "--current"],
    ["workspaces", "--current"], ["receipt", "operation", "--current"],
    ["artifact", "operation", "--current", "--output", "/tmp/must-not-write"],
  ]) {
    const client = fixture();
