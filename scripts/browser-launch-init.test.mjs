...
test("startup files cross the remote boundary as ordered contents in one creation", async () => {
  ...
  try {
    ...
    const result = await f.run(["create", "--workspace", "workspace", "--idempotency-key", "launch-once", "--init-script", "한글 file.js", "--init-script=empty.js", "--init-script", "한글 file.js"]);
    assert.equal(result.ok, true, JSON.stringify(result));
    assert.deepEqual(f.calls, [{ kind: "create", workspace_id: "workspace", operation_id: "launch-once", init_scripts: [source, "", source] }]);
...
test("native startup environment files precede repeated explicit files", async () => {
  ...
  try {
    ...
    assert.equal((await f.run(["create", "--workspace", "workspace", "--init-script", "last.js"])).ok, true);
    assert.deepEqual(f.calls[0].init_scripts, ["first", "second", "last"]);
    const plain = fixture(root);
    assert.equal((await plain.run(["create", "--workspace", "workspace"])).ok, true);
...
test("invalid, unreadable, non-UTF8 and oversized startup files fail before backend contact", async () => {
  ...
  try {
    ...
    for (const files of [["missing.js"], ["."], ["invalid.js"], ["large.js"], ["half.js", "half.js"], Array(17).fill("half.js"), [""]]) {
      const f = fixture(root);
      const result = await f.run(["create", "--workspace", "workspace", ...files.flatMap(file => ["--init-script", file])]);
