...
const resources = [0, 1, 128].map((i) => ({ workspace_id: workspaceId, resource_id: `r:${i}`, generation: "g" }));
...
function fixture(change = (_body, value) => value) {
  const calls = [];
  const run = (args = ["tab", "current", "--all"]) => collectBrowserCommand({
    ...
    requestBackend: async (profile, request) => {
      ...
      let value;
      if (body.kind === "list") {
        value = { workspace_id: workspaceId, resources: resources.map((resource) => ({ resource })), target: { workspace_id: workspaceId, generation: "g", revision: "1", current_resource: resources[0] } };
...
test("shared current returns the first Host current tab in full catalog order", async () => {
...
test("current skips resources without Host selection and retains a later resource identity", async () => {
...
test("no Host current tab or an empty catalog never falls back to an arbitrary page", async () => {
  for (const empty of [false, true]) {
    const f = fixture((body, value) => empty && body.kind === "list" ? { ...value, resources: [], target: { ...value.target, current_resource: null } }
...
test("a later catalog or observation failure rejects the read despite an earlier current candidate", async () => {
  for (const fault of ["generation", "renderer"]) {
...
test("shared current keeps the single aggregate deadline", async () => {
  let now = 1000; vi.spyOn(Date, "now").mockImplementation(() => now);
  const f = fixture((body, value) => { if (body.kind === "list") now += 45_001; return value; });
  const result = await f.run();
  assert.equal(result.error?.code, "browser_tab_list_timeout", JSON.stringify(result));
  assert.deepEqual(f.calls, [{ kind: "list" }]);
});

test("shared current cannot accept another target, page, profile display or mutation flags", async () => {
  for (const extra of [["r:0"], ["--resource", "r:0"], ["--workspace", "w:000"], ["--page", "current"], ["--show-profile"], ["--controller", "agent"], ["--epoch", "1"], ["--index", "0"]]) {
    const f = fixture(); const result = await f.run(["tab", "current", "--all", ...extra]);
