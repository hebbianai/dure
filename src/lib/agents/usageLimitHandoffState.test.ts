...
it("permits explicit recovery after failure and deduplicates pending manual attempts", () => {
	const agentId = "handoff-state-recovery";
	const first = state.begin(agentId, 10)!;
	expect(state.begin(agentId, 10)).toBeUndefined();
	expect(state.begin(agentId, 10)).toBeUndefined();
	...
	const recovery = state.begin(agentId, 10)!;
...
it("keeps a newer failure visible when an older transition settles late", () => {
	...
	const first = state.begin(agentId, 10)!;
	const second = state.begin(agentId, 20)!;
