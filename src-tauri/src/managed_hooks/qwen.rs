...
const HOOK: &str = include_str!("../../resources/managed-qwen-hook.mjs");
const EXTENSION: &str = include_str!("../../resources/managed-qwen-extension.mjs");
...
pub(super) fn publish(
    ...
    if !runtime.is_absolute() {
        return Err(std::io::Error::other(
            "managed Qwen runtime must be absolute",
        ));
    }
    ...
    write_owner_only_file(
        control_dir,
        &control_dir.join("managed-qwen-hook.mjs"),
        hook.as_bytes(),
        0o600,
    )?;
    write_owner_only_file(
    ...
    let extension = control_dir.join("managed-qwen-extension.mjs");
    write_owner_only_file(control_dir, &extension, EXTENSION.as_bytes(), 0o600)?;
    let launcher = control_dir.join("managed-qwen-launch.sh");
    let script = format!(
        "#!/bin/sh\nset -eu\nhook=$(node {})\nexport DURE_QWEN_HOOK_PATH=\"$hook\"\nexec \"$@\"\n",
...
mod tests {
    ...
    #[test]
    fn published_qwen_launch_preserves_arguments_and_original_settings() {
        ...
        write_owner_only_file(
            root,
            &probe,
            b"#!/bin/sh\nprintf '%s\\0' \"$QWEN_CODE_SYSTEM_DEFAULTS_PATH\" \"$DURE_QWEN_HOOK_PATH\" \"$@\"\n",
            0o700,
        )
        .unwrap();
        let original = vec![
        ...
        dure_provider_adapter::apply_provider_runtime_integration(
            &dure_app::ProviderIdV1::new("qwen-code").unwrap(),
            &integration,
            &mut plan,
        )
        .unwrap();
        let output = std::process::Command::new(plan.executable)
            .args(plan.arguments)
            .env("QWEN_CODE_SYSTEM_DEFAULTS_PATH", &defaults)
            .env("QWEN_HOME", root.join("home"))
            .output()
            .unwrap();
        assert!(
        ...
        assert_eq!(
            ...
            std::fs::canonicalize(root.join("managed-qwen-hook.mjs")).unwrap(),
        );
        assert!(root.join("home/extensions/dure-lifecycle-v1/hooks/hooks.json").is_file());
