...
Dure plugins add app integrations such as issue-tracker views and workspace settings. Open **Plugins** in the sidebar to see the catalog. This entry is available in both the Basic and Beta interfaces.
...
For example, GitHub provides a workspace refresh interval and default issue list. Its contributed **GitHub** sidebar opens repository work; the **Plugins** catalog configures the integration itself.

## App plugins and agent tooling

| Surface | Use it for |
| --- | --- |
| Plugins sidebar | Dure integration catalog, compatibility and settings |
| GitHub sidebar | Repository issues, pull requests, Projects and task starts |
| Settings → Agent CLI | Installed coding-agent CLIs and provider setup |
